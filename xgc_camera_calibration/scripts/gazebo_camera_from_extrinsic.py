#!/usr/bin/env python3
"""Exec Gazebo camera roslaunch with an explicitly selected physical pose."""
...
def main(argv=None):
    ...
    parser.add_argument(
        "--pose-source", choices=("authored", "physical-selection"), required=True
    )
    ...
    if args.pose_source == "physical-selection":
        pose, selected = resolve_gazebo_camera_initial_pose(
            args.calibration_root,
            args.camera_name,
            args.parent_frame,
            args.selected_physical_optical_frame,
            (args.optical_offset_x, args.optical_offset_y, args.optical_offset_z),
        )
        launch = replace_roslaunch_pose_arguments(launch, pose)
        print(
            "Applying shared physical extrinsic {} as Gazebo camera initial pose".format(
                selected
            ),
            flush=True,
        )
